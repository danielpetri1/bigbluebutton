...
class MeetingActor(
  ...
  private def handleMessageThatAffectsInactivity(msg: BbbCommonEnvCoreMsg): Unit = {
    ...
    msg.core match {
      ...
      case m: BreakoutSnapshotReqMsg              => presentationPodsApp.handle(m, state, liveMeeting, msgBus)
      ...
      case m: NewBreakoutSnapshotFileAvailableMsg            => presentationPodsApp.handle(m, liveMeeting, msgBus)
