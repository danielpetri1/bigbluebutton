...
trait BreakoutRoomCreatedMsgHdlr {
  ...
  def sendBreakoutRoomsList(breakoutModel: BreakoutModel): BreakoutModel = {
    val breakoutRooms = breakoutModel.rooms.values.toVector map { r =>
      ...
      new BreakoutRoomInfo(r.name, r.externalId, r.id, r.sequence, r.shortName, r.isDefaultName, r.freeJoin, html5JoinUrls.toMap, r.captureNotes, r.captureSlides, r.defaultNotesText)
    }
  ...
  def sendBreakoutRoomStarted(room: BreakoutRoom2x): BreakoutRoom2x = {
    ...
    val breakoutInfo = BreakoutRoomInfo(room.name, room.externalId, room.id, room.sequence, room.shortName, room.isDefaultName, room.freeJoin, Map(), room.captureNotes, room.captureSlides, room.defaultNotesText)
