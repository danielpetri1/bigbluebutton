...
module BigBlueButton
  def self.process_webcam_videos(target_dir, raw_archive_dir, output_width, output_height, output_framerate, audio_offset, processed_audio_file, video_formats = ['webm'])
  ...
  def self.process_deskshare_videos(target_dir, raw_archive_dir, output_width, output_height, output_framerate, video_formats = ['webm'])
    ...
    return unless BigBlueButton.video_recorded?(deskshare_video_edl)
  ...
  def self.video_recorded?(video_edl)
    video_edl.each do |edl|
      edl[:areas].each do |_name, videos|
        return true unless videos.empty?
      end
    end
