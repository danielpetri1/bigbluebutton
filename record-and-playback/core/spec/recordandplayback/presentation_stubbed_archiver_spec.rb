  ...
  describe PresentationArchiver do
    context '#success' do
      it 'should copy presentations to archive' do
        ...
        FileUtils.stub(:cp_r)
        Dir.stub(:glob).and_return(%w[flight-school ecosystem])
        from_dir = 'from'
        to_dir = 'to'
    ...
    context '#fail' do
      it 'should raise from directory not found exception' do
        ...
        FileUtils.stub(:cp_r)
        Dir.stub(:glob).and_return(%w[flight-school ecodystem])
        from_dir = '/from-dir-not-found'
        to_dir = 'resources/archive'
        meeting_id = 'meeting-id'

        expect do
          BigBlueButton::PresentationArchiver.archive(meeting_id, from_dir, to_dir)
        end.to raise_error(MissingDirectoryException)
      end
      ...
      it 'should raise presentation files not found exception' do
        ...
        expect do
          BigBlueButton::PresentationArchiver.archive(meeting_id, from_dir, to_dir)
        end.to raise_error(FileNotFoundException)
      end
    end
  end
end
